import useTheme from '../../Hooks/useTheme'

//SASS
import './Header.scss'

//IMGES
import Logo from '../../Assets/imgs/youtube_logo.svg'

function Header() {
    let [theme, setTheme] = useTheme()

    let elBody = document.querySelector('body')
    elBody.classList = theme


    function changeTheme() {
        if (theme === 'light') {
            setTheme('dark')
        } else {
            setTheme('light')
        }
    }

    return (
        <header className='header'>
            <div style={{ display: 'none' }}>
                {/* bar icon */}
                <svg width="20" height="17" className="hidden">
                    <symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 17" id="bar_icon">
                        <path fillRule="evenodd" clipRule="evenodd" d="M18.6364 3H1.36364C0.61 3 0 2.328 0 1.5C0 0.672 0.61 0 1.36364 0H18.6364C19.3891 0 20 0.672 20 1.5C20 2.328 19.3891 3 18.6364 3ZM1.36364 7H18.6364C19.3891 7 20 7.672 20 8.5C20 9.328 19.3891 10 18.6364 10H1.36364C0.61 10 0 9.328 0 8.5C0 7.672 0.61 7 1.36364 7ZM18.6364 14H1.36364C0.61 14 0 14.672 0 15.5C0 16.328 0.61 17 1.36364 17H18.6364C19.3891 17 20 16.328 20 15.5C20 14.672 19.3891 14 18.6364 14Z"></path>
                    </symbol>
                </svg>
                {/* theme icon */}
                <svg width="22" height="22" className="hidden">
                    <symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 22" id="theme_icon">
                        <path d="M22.772 9.50599L17.154 7.31399L14.994 0.813988C14.892 0.506988 14.604 0.299988 14.282 0.299988C13.96 0.299988 13.672 0.506988 13.57 0.812988L11.41 7.31299L5.79 9.50499C5.503 9.61699 5.313 9.89499 5.313 10.205C5.313 10.515 5.503 10.79 5.791 10.903L11.411 13.095L13.571 19.595C13.673 19.901 13.961 20.108 14.283 20.108C14.605 20.108 14.893 19.901 14.995 19.595L17.155 13.095L22.775 10.903C23.062 10.791 23.252 10.513 23.252 10.203C23.252 9.89299 23.062 9.61799 22.774 9.50599H22.772ZM16.282 11.826C16.074 11.906 15.912 12.076 15.842 12.286L14.282 16.981L12.722 12.288C12.652 12.078 12.492 11.908 12.284 11.826L8.129 10.206L12.283 8.58399C12.491 8.50399 12.653 8.33399 12.723 8.12199L14.283 3.42899L15.843 8.12299C15.913 8.33499 16.073 8.50499 16.281 8.58599L20.436 10.206L16.281 11.828L16.282 11.826ZM6.663 2.81199H4.783V1.04999C4.783 0.635988 4.446 0.299988 4.033 0.299988C3.62 0.299988 3.283 0.635988 3.283 1.04999V2.81199H1.5C1.086 2.81199 0.75 3.14799 0.75 3.56199C0.75 3.97599 1.086 4.31199 1.5 4.31199H3.282V6.07399C3.282 6.48799 3.618 6.82399 4.032 6.82399C4.446 6.82399 4.782 6.48799 4.782 6.07399V4.31199H6.662C7.077 4.31199 7.412 3.97599 7.412 3.56199C7.412 3.14799 7.077 2.81199 6.662 2.81199H6.663ZM9.198 18.434H8.098V17.418C8.098 17.004 7.763 16.668 7.348 16.668C6.933 16.668 6.598 17.004 6.598 17.418V18.434H5.57C5.156 18.434 4.82 18.77 4.82 19.184C4.82 19.598 5.156 19.934 5.57 19.934H6.6V20.95C6.6 21.364 6.935 21.7 7.35 21.7C7.765 21.7 8.1 21.364 8.1 20.95V19.934H9.198C9.612 19.934 9.948 19.598 9.948 19.184C9.948 18.77 9.612 18.434 9.198 18.434Z"></path>
                    </symbol>
                </svg>
                {/* dots icon */}
                <svg width="21" height="21" className="hidden">
                    <symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 21" id="dots_icon">
                        <path  fillRule="evenodd" clipRule="evenodd" d="M0 2.5C0 3.88071 1.11929 5 2.5 5C3.88071 5 5 3.88071 5 2.5C5 1.11929 3.88071 0 2.5 0C1.11929 0 0 1.11929 0 2.5ZM2.5 13C1.11929 13 0 11.8807 0 10.5C0 9.11929 1.11929 8 2.5 8C3.88071 8 5 9.11929 5 10.5C5 11.8807 3.88071 13 2.5 13ZM2.5 21C1.11929 21 0 19.8807 0 18.5C0 17.1193 1.11929 16 2.5 16C3.88071 16 5 17.1193 5 18.5C5 19.8807 3.88071 21 2.5 21ZM10.5 21C9.11929 21 8 19.8807 8 18.5C8 17.1193 9.11929 16 10.5 16C11.8807 16 13 17.1193 13 18.5C13 19.8807 11.8807 21 10.5 21ZM16 18.5C16 19.8807 17.1193 21 18.5 21C19.8807 21 21 19.8807 21 18.5C21 17.1193 19.8807 16 18.5 16C17.1193 16 16 17.1193 16 18.5ZM10.5 13C9.11929 13 8 11.8807 8 10.5C8 9.11929 9.11929 8 10.5 8C11.8807 8 13 9.11929 13 10.5C13 11.8807 11.8807 13 10.5 13ZM8 2.5C8 3.88071 9.11929 5 10.5 5C11.8807 5 13 3.88071 13 2.5C13 1.11929 11.8807 0 10.5 0C9.11929 0 8 1.11929 8 2.5ZM18.5 13C17.1193 13 16 11.8807 16 10.5C16 9.11929 17.1193 8 18.5 8C19.8807 8 21 9.11929 21 10.5C21 11.8807 19.8807 13 18.5 13ZM16 2.5C16 3.88071 17.1193 5 18.5 5C19.8807 5 21 3.88071 21 2.5C21 1.11929 19.8807 0 18.5 0C17.1193 0 16 1.11929 16 2.5Z" ></path>
                    </symbol>
                </svg>
                {/* rang icon */}
                <svg width="22" height="26" className="hidden">
                    <symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 26" id="rang_icon">
                        <path fillRule="evenodd" clipRule="evenodd" d="M3.66927 22.4545C1.64382 22.4545 0 20.8655 0 18.9091C0 17.6106 0.727122 16.4532 1.83849 15.835C1.83849 15.4764 1.83333 11.2256 1.83333 11.2256C1.83333 7.14185 4.66852 3.70365 8.55556 2.6765V2.36364C8.55556 1.05824 9.64997 0 11 0C12.35 0 13.4444 1.05824 13.4444 2.36364V2.6765C17.3323 3.70344 20.1667 7.14194 20.1667 11.2318V15.8454C21.2793 16.4664 22 17.619 22 18.9091C22 20.8681 20.3586 22.4545 18.3307 22.4545H14.6667C14.6646 24.413 13.0197 26 11 26C8.97496 26 7.33333 24.4143 7.33333 22.4571L3.66927 22.4545ZM9.77778 22.4545C9.77778 23.1083 10.3244 23.6364 11 23.6364C11.6709 23.6364 12.2201 23.1064 12.2222 22.4545H9.77778ZM17.7222 11.2318V16.9427C17.7222 17.2702 17.9922 17.6032 18.3181 17.6847L18.6385 17.7648C19.1733 17.8984 19.5556 18.3689 19.5556 18.9091C19.5556 19.5624 19.0088 20.0909 18.3307 20.0909H3.66927C2.99369 20.0909 2.44444 19.56 2.44444 18.9091C2.44444 18.3666 2.82517 17.8979 3.36026 17.7647L3.69151 17.6823C4.0153 17.6017 4.27778 17.2732 4.27778 16.9426V11.2256C4.27778 7.63639 7.28675 4.72727 11 4.72727C14.7148 4.72727 17.7222 7.63541 17.7222 11.2318Z"></path>
                    </symbol>
                </svg>
                {/* lupa */}
                <svg width="19" height="19" className="hidden">
                    <symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 19 19" id="lupa">
                        <path fillRule="evenodd" clipRule="evenodd" d="M17.9005 16.4865L14.3205 12.9065L14.3195 12.9055C16.9095 9.569 16.4594 4.79387 13.2916 2.00007C10.1238 -0.793727 5.32983 -0.643497 2.34317 2.34317C-0.643497 5.32983 -0.793727 10.1238 2.00007 13.2916C4.79387 16.4594 9.569 16.9095 12.9055 14.3195L16.4855 17.8995C16.7364 18.1596 17.1081 18.2642 17.4578 18.1729C17.8075 18.0816 18.0808 17.8088 18.1725 17.4592C18.2643 17.1097 18.1603 16.7378 17.9005 16.4865ZM13.1967 11.0005C12.1249 12.8569 10.1441 14.0005 8.00052 14.0005C4.68681 14.0005 2.00052 11.3142 2.00052 8.00052C2.00052 4.68681 4.68681 2.00052 8.00052 2.00052C10.1441 2.00052 12.1249 3.14411 13.1967 5.00052C14.2685 6.85693 14.2685 9.14411 13.1967 11.0005Z"></path>
                    </symbol>
                </svg>
            </div>


            <nav className={`header__nav ${theme}`}>
                <div className="header__box">
                    <button className='header__bar'>
                        <svg className={`icon ${theme}`}>
                            <use xlinkHref="#bar_icon"></use>
                        </svg>
                    </button>
                    <div className='header__logo--box'>
                        <img className='header__logo' src={Logo} alt="Logo" />
                        <p className={`header__logo--text ${theme}`}>YouTube</p>
                    </div>
                    <div className='header__inputbox'>
                        <input className='header__input' type="text" placeholder='Search' />
                        <svg className="header__lupa icon">
                            <use xlinkHref="#lupa"></use>
                        </svg>
                    </div>
                </div>
                <div className='header__subbox'>
                    <button className='header__camera--btn' onClick={changeTheme}>
                        <svg className={`icon ${theme}`}>
                            <use xlinkHref="#theme_icon"></use>
                        </svg>
                    </button>
                    <button className='header__dots--btn'>
                        <svg className={`icon ${theme}`}>
                            <use xlinkHref="#dots_icon"></use>
                        </svg>
                    </button>
                    <button className='header__rang--btn'>
                        <svg className={`icon ${theme}`}>
                            <use xlinkHref="#rang_icon"></use>
                        </svg>
                    </button>
                    <button className='header__pic--btn'>B</button>
                </div>
            </nav>
        </header>
    )
}

export default Header